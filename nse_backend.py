        ...
        def fetch_nse_endpoint(key_url):
            ...
            try:
                ...
                if r.ok:
                    j = r.json()
                    items = _nse_rows(j)
                    if key=="circuits" and not items:        # fallback: price-band-hitters shape  upper -> AllSec -> data
                        try:
                            r2 = session.get("https://www.nseindia.com/api/live-analysis-price-band-hitters", headers=NSE_HEADERS, timeout=10)
                            if r2.ok:
                                j2 = r2.json()
                                items = [x for x in ((((j2.get("upper") or {}).get("AllSec") or {}).get("data")) or []) if isinstance(x,dict)]
                        except Exception as _e2: print(f"[WARN] circuits fallback: {_e2}")
                    out=[]
                    for i in items:
...
def hot_diag():
    ...
    urls={"gainers":"https://www.nseindia.com/api/live-analysis-variations?index=gainers",
          ...
          "active":"https://www.nseindia.com/api/live-analysis-most-active-securities?index=volume",
          "circuits":"https://www.nseindia.com/api/live-analysis-data-for-alerts?index=uppercircuit",
          "price_band_hitters":"https://www.nseindia.com/api/live-analysis-price-band-hitters",
