...
def news():
    """Get catalyst context from Claude API for given symbols."""
    import os
    ...
    try:
        ...
        msg = client.messages.create(
            ...
            max_tokens=1000,
            system='You are an NSE analyst. For each symbol give a brief catalyst. Return ONLY JSON: {"SYMBOL":{"headline":"under 90 chars","source":"source","pattern":"pattern"}}. No markdown.',
            messages=[{"role":"user","content":f"Give catalyst context for: {', '.join(symbols)}. JSON only."}]
        )
        text = msg.content[0].text
        s, e = text.find("{"), text.rfind("}")
        if s != -1 and e > s:
            import json
            return jsonify(json.loads(text[s:e+1]))
    except Exception as ex:
        print(f"News error: {ex}")
    return jsonify({})
