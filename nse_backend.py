...
import os, re, json, math, time, threading, secrets, gc, random
...
WATCHLIST = list(dict.fromkeys([
    ...
    "TECHM","ULTRACEMCO","ONGC","NTPC","POWERGRID","SUNPHARMA","TMPV","TMCV","HCLTECH",
    ...
    "ADANIENT","ADANIPORTS","LTM","INDUSINDBK","ITC","VEDL","GRASIM","TATACONSUM",
    ...
    "BIOCON","ETERNAL","DMART","DIXON","VOLTAS","POLYCAB","BALKRISIND","APOLLOTYRE",
    "MRF","INDIGO","CONCOR","TATACOMM","ANGELONE","CDSL","MCX","CAMS","NAUKRI","BSE",
    "LICI","MAXHEALTH","FORTIS","IRFC","RVNL",
    # IT & TECH
    "KPITTECH","MPHASIS","LTTS","PERSISTENT","COFORGE","BSOFT","MASTEK","RAMCOSYS",
    "TANLA","ROUTE","CYIENT","SONATSOFTW","INTELLECT","ZENSARTECH","TATAELXSI",
    "FSL","RATEGAIN","NAZARA","DATAMATICS","SAKSOFT","TATATECH",
    # BANKING & FINANCE
    "RBLBANK","ABCAPITAL","SUNDARMFIN","CANFINHOME","AAVAS","MANAPPURAM",
    "MOTILALOFS","IIFL","PNBHOUSING","LICHSGFIN","CREDITACC","SPANDANA",
    "SBFC","UGROCAP","M&MFIN","REPCOHOME","APTUS","ARMANFIN","EQUITASBNK","UJJIVANSFB",
    "BANKBARODA","PNB","CANBK","UNIONBANK","YESBANK","HDFCAMC","SBICARD","MFSL","KFINTECH",
    # PHARMA & HEALTHCARE
    "GRANULES","GLENMARK","NATCOPHARM","IPCALAB","ALEMBICLTD",
    "NEULANDLAB","SUVEN","WOCKPHARMA","AJANTPHARM","SHILPAMED","KRSNAA",
    "MEDPLUS","LALPATHLAB","METROPOLIS","THYROCARE","GLAXO","PFIZER","ABBOTINDIA",
    "SANOFI","CAPLIPOINT","SOLARA","MANKIND","LAURUSLABS","GLAND",
    # DEFENCE & AEROSPACE
    "BEML","GRSE","COCHINSHIP","MAZDOCK","MIDHANI","MTARTECH",
    "DATAPATTNS","HBLENGINE","SOLARINDS","ZENTEC","PARAS","BDL",
    # CHEMICALS & SPECIALTY
    "DEEPAKNTR","NAVINFLUOR","TATACHEM","CLEAN","FINEORG","ALKYLAMINE",
    "VINATIORGA","AARTIIND","DEEPAKFERT","GNFC","CHAMBLFERT","COROMANDEL","PIIND",
    ...
    "FIEMIND","GABRIEL","JAMNAAUTO","LUMAXTECH","UNOMINDA","SUBROS","CEATLTD","TVSMOTOR","MUNJALAU","FMGOETZE",
    # INFRASTRUCTURE & CAPEX
    "GMRAIRPORT","CUMMINSIND","ASTRAL","RAILTEL","CAPACITE","KNRCON","HGINFRA",
    ...
    # METALS & MINING
    "APLAPOLLO","RATNAMANI","GRAPHITE","HINDCOPPER","NATIONALUM","MOIL","WELCORP","SARDAEN",
    # FMCG & CONSUMER
    "RADICO","JUBLFOOD","TTKPRESTIG","HAWKINCOOK","SYMPHONY","WONDERLA","VBL","UBL","EMAMILTD",
    "PAGEIND","RAYMOND","SAFARI","VMART","SENCO","KALYANKJIL","BATAINDIA","UNITDSPR","EMIL",
    ...
    # LOGISTICS
    "DELHIVERY","ALLCARGO","TCIEXP","VRLLOG","SNOWMAN","BLUEDART",
    # AGRI & FOOD
    "KRBL","AVANTIFEED","BALRAMCHIN","TRIVENI","RENUKA","PRAJIND","KHAITANLTD",
    "LTFOODS","AWL","PATANJALI","PARADEEP","FACT",
    # CEMENT
    "JKCEMENT","DALBHARAT","RAMCOCEM","HEIDELBERG","BIRLACORPN","ACC",
    # NEW-AGE
...
# ── SECTOR MAP ─────────────────────────────────────────────────────────────
SECTOR_STOCKS = {
    "Technology & AI":         ["TCS","INFY","HCLTECH","WIPRO","TECHM","LTM","PERSISTENT","COFORGE","KPITTECH","MPHASIS"],
    "Semiconductors":          ["DIXON","KAYNES","SYRMA","AMBER","TATAELXSI","BEL","HAL","BEML"],
    "Defence & Aerospace":     ["HAL","BEL","BHEL","BEML","GRSE","COCHINSHIP","MAZDOCK"],
    ...
    "EV & Auto":               ["TMPV","TMCV","MARUTI","BAJAJ-AUTO","HEROMOTOCO","EICHERMOT","MOTHERSON","BOSCHLTD"],
    "Renewable Energy":        ["ADANIGREEN","TATAPOWER","ADANIPOWER","NHPC","SJVN","IREDA","NTPC"],
    "Infrastructure & Capex":  ["LT","RVNL","IRFC","IRCTC","RAILTEL","GMRAIRPORT","KNRCON","HGINFRA"],
    ...
    "Cement":                  ["ULTRACEMCO","AMBUJACEM","SHREECEM","JKCEMENT","DALBHARAT","RAMCOCEM"],
...
def compute_advanced_signals(daily, regime=None):
    ...
    out = {"ret_20d": None, "rs_score": None, "vcp_setup": False, "vcp_contractions": 0, "rs_raw": None, "mom_12_1": None}
    try:
        ...
        def _r(lb): return (float(close.iloc[-1])/float(close.iloc[-1-lb])-1)*100 if len(close)>lb else None
        parts=[(w,_r(lb)) for w,lb in ((0.4,63),(0.2,126),(0.2,189),(0.2,252)) if _r(lb) is not None]
        if len(close)>126 and parts: out["rs_raw"]=round(sum(w*v for w,v in parts)/sum(w for w,_ in parts),2)
        if len(close)>253: out["mom_12_1"]=round((float(close.iloc[-22])/float(close.iloc[-253])-1)*100,2)
...
def compute_indicators(df, intra=None):
    ...
    try:
        ...
        # ── Minervini Trend Template / Weinstein stage (needs ~1y of daily bars) ──
        try:
            n=len(close)
            sma50=float(close.rolling(50).mean().iloc[-1]) if n>=50 else None
            sma150=float(close.rolling(150).mean().iloc[-1]) if n>=150 else None
            s200=close.rolling(200).mean() if n>=200 else None
            sma200=float(s200.iloc[-1]) if s200 is not None else None
            sma200_prev=float(s200.iloc[-22]) if (s200 is not None and n>=222) else None
            r["sma50"]=round(sma50,2) if sma50 else None; r["sma200"]=round(sma200,2) if sma200 else None
            r["above_sma50"]=bool(sma50 and r["cmp"]>sma50); r["above_sma200"]=bool(sma200 and r["cmp"]>sma200)
            lo52=float(low.iloc[-252:].min()); hi52=float(high.iloc[-252:].max())
            conds=[]
            if sma50 and sma150 and sma200:
                conds=[r["cmp"]>sma150 and r["cmp"]>sma200, sma150>sma200,
                       bool(sma200_prev is not None and sma200>sma200_prev), sma50>sma150>sma200,
                       r["cmp"]>sma50, r["cmp"]>=lo52*1.3, r["cmp"]>=hi52*0.75]
            r["tt_score"]=int(sum(bool(x) for x in conds)) if conds else None
            r["trend_template"]=bool(conds and all(conds))
            r["stage4"]=bool(sma200 and sma200_prev is not None and r["cmp"]<sma200 and sma200<sma200_prev)
        except Exception:
            r["tt_score"]=None; r["trend_template"]=False; r["stage4"]=False; r["above_sma50"]=False; r["above_sma200"]=False
        try:
            if intra is not None and len(intra)>=3:
                ih=intra["High"].squeeze().astype(float); il=intra["Low"].squeeze().astype(float)
                ic=intra["Close"].squeeze().astype(float); iv=intra["Volume"].squeeze().astype(float)
                vwap_v=float((((ih+il+ic)/3)*iv).cumsum().iloc[-1]/iv.cumsum().iloc[-1])
            else:
                tp=(high+low+close)/3; vwap_v=float((tp*volume).iloc[-5:].sum()/volume.iloc[-5:].sum())
        except Exception: vwap_v=float(close.iloc[-1])
        r["vwap"]=round(vwap_v,2); r["above_vwap"]=bool(r["cmp"]>r["vwap"])
        base=volume.iloc[-51:-1] if len(volume)>=52 else volume.iloc[:-1]   # 50-day average, excluding today
        avg_vol=float(base.mean()) if len(base)>0 else float(volume.mean())
        vol_now=float(volume.iloc[-1]); frac=1.0
        try:
            if intra is not None and len(intra)>=3 and _is_market_open(): frac=max(0.15,min(1.0,len(intra)/75.0))
        except Exception: pass
        r["volume"]=int(vol_now); r["avg_volume"]=int(avg_vol)
        r["rel_volume"]=round((vol_now/frac)/avg_vol,2) if avg_vol>0 else 1.0   # today's volume projected to a full session
        ...
        r["week52_high"]=round(float(high.iloc[-252:].max()),2); r["week52_low"]=round(float(low.iloc[-252:].min()),2)
...
def score_stock(ind, regime=None, results_info=None):
    ...
    rs=ind.get("rs_score") if ind.get("rs_rank") is None else None   # 20d RS only as fallback
    if rs is not None:
    ...
    rk=ind.get("rs_rank")
    if rk is not None:
        if rk>=90:   score+=2.5; signals.append(f"RS rank {rk}/99 — market leader (12-mo weighted) ✓")
        elif rk>=80: score+=1.5; signals.append(f"RS rank {rk}/99 — strong leader ✓")
        elif rk>=70: score+=0.5; signals.append(f"RS rank {rk}/99 — above average")
        elif rk<=20: score-=1.5; signals.append(f"RS rank {rk}/99 — laggard ✗")
        elif rk<=35: score-=0.5; signals.append(f"RS rank {rk}/99 — weak ✗")
        else: signals.append(f"RS rank {rk}/99 — middle of pack")
    if ind.get("trend_template"): score+=2.0; signals.append("Minervini Trend Template 7/7 — Stage 2 uptrend ✓")
    elif ind.get("stage4"):      score-=2.0; signals.append("Below a falling 200-day average — Stage 4 downtrend ✗")
    elif (ind.get("tt_score") or 0)>=6: score+=0.5; signals.append(f"Trend Template {ind.get('tt_score')}/7 — nearly Stage 2")
...
def fetch_nifty_regime():
    try:
        ...
        vix=vix_chg=None
        try:
            vd=yf.Ticker("^INDIAVIX").history(period="5d",interval="1d")
            if vd is not None and len(vd)>=2: vix=round(float(vd["Close"].iloc[-1]),2); vix_chg=round((vix/float(vd["Close"].iloc[-2])-1)*100,1)
        except Exception: pass
        a200=bool(len(close)>=200 and cmp>float(close.rolling(200).mean().iloc[-1]))
        return {"trend":trend,"strength":strength,"vix":vix,"vix_chg":vix_chg,"above_sma200":a200,"ema20":round(ema20,2),"ema50":round(ema50,2),"cmp":round(cmp,2),"change_pct":chg,"nifty_ret_20d":nifty_ret_20d}
    ...
    try:
        ...
        for _att in range(3):
            try:
                ...
                daily = prefetched_daily if prefetched_daily is not None and len(prefetched_daily)>=60                         else ticker.history(period="1y",interval="1d",auto_adjust=True)
                break
            except Exception as _re:
                if "429" in str(_re) or "rate" in str(_re).lower() or "crumb" in str(_re).lower() or "401" in str(_re): time.sleep(2**_att)
                else: raise
        df=daily if (daily is not None and len(daily)>=26) else intra
        ind=compute_indicators(df,intra)
...
def _pct_rank(sorted_vals, v):
    import bisect
    if v is None or len(sorted_vals)<20: return None
    return int(round(bisect.bisect_left(sorted_vals, v)/len(sorted_vals)*99))
def rs_percentile(raw):
    return _pct_rank(sorted(x["rs_raw"] for x in get_full_scan_snapshot().values() if x.get("rs_raw") is not None), raw)

def _finalize_scan_context(results, regime):
    """After a full scan: RS percentile rank -> re-score, breadth, exposure dial, daily pick log."""
    try:
        raws=sorted(r["rs_raw"] for r in results if r.get("rs_raw") is not None)
        for r in results:
            ind=r.get("_ind")
            if ind is None: continue
            rk=_pct_rank(raws, r.get("rs_raw")); ind["rs_rank"]=rk; r["rs_rank"]=rk
            sc,sg=score_stock(ind,regime,get_results_info(r["symbol"])); r["score"]=sc; r["signals"]=sg
        b50=[bool(r.get("above_sma50")) for r in results if "above_sma50" in r]
        b200=[bool(r.get("above_sma200")) for r in results if r.get("sma200") is not None]
        regime["breadth50"]=round(100*sum(b50)/len(b50)) if b50 else None
        regime["breadth200"]=round(100*sum(b200)/len(b200)) if b200 else None
        regime["adv_pct"]=round(100*sum(1 for r in results if (r.get("change_pct") or 0)>0)/len(results)) if results else None
        pts={"BULL":2,"NEUTRAL":1,"BEAR":0}.get(regime.get("trend"),1)
        if regime.get("breadth50") is not None: pts+=2 if regime["breadth50"]>=55 else 1 if regime["breadth50"]>=40 else 0
        if regime.get("breadth200") is not None and regime["breadth200"]>=50: pts+=1
        v=regime.get("vix")
        if v is not None: pts+=1 if v<16 else 0 if v<22 else -1
        ex="FULL" if pts>=5 else "REDUCED" if pts>=3 else "DEFENSIVE"
        regime["exposure"]=ex
        regime["exposure_note"]={"FULL":"Healthy market — normal position sizes are fine.",
            "REDUCED":"Mixed market — halve position sizes, prefer Trend-Template leaders.",
            "DEFENSIVE":"Weak market — mostly cash; only the very best setups, small size."}[ex]
        log_top_picks(results, regime)
    except Exception as e: print(f"[CTX ERR] {e}")

# ── SIGNAL SCORECARD (forward test): logs the day's top-20 once after 3 PM IST and measures
# 1/3/5/10-day forward returns vs NIFTY, overall and per signal. Stored on local disk —
# Render's free tier wipes it on redeploy/restart; set SIGNAL_LOG_PATH to a persistent disk to keep history.
_LOG_PATH=os.environ.get("SIGNAL_LOG_PATH","/tmp/mm_signal_log.json"); _log_lock=threading.Lock()
_score_cache={"t":0,"data":None}
def _load_log():
    try:
        with open(_LOG_PATH) as f: return json.load(f)
    except Exception: return []
def log_top_picks(results, regime):
    try:
        now=datetime.now(IST)
        if now.weekday()>=5 or now.hour<15 or (now.hour==15 and now.minute>45) or now.hour>=16: return
        today=now.strftime("%Y-%m-%d")
        with _log_lock:
            log=_load_log()
            if any(e.get("date")==today for e in log): return
            picks=[]
            for r in sorted(results,key=lambda x:x.get("score",0),reverse=True)[:20]:
                t=[k for k,c in (("trend_template",r.get("trend_template")),("vcp",r.get("vcp_setup")),
                    ("rs_top20pct",(r.get("rs_rank") or 0)>=80),("breakout",r.get("breakout_setup")),
                    ("delivery_accum",(r.get("delivery_pct") or 0)>=65 and (r.get("change_pct") or 0)>0),
                    ("supertrend",r.get("supertrend_bull")),("macd",r.get("macd_bullish"))) if c]
                picks.append({"s":r["symbol"],"p":r.get("cmp"),"sc":r.get("score"),"tags":t})
            log.append({"date":today,"exposure":regime.get("exposure"),"picks":picks})
            with open(_LOG_PATH,"w") as f: json.dump(log[-120:],f)
    except Exception as e: print(f"[LOG ERR] {e}")

def compute_scorecard():
    import pandas as pd
    log=_load_log()
    if not log: return {"status":"empty","days":0,"message":"No picks logged yet — the first snapshot is saved after 3 PM IST on a trading day."}
    syms=sorted({p["s"] for e in log for p in e["picks"]}); closes={}
    for i in range(0,len(syms),50):
        for sy,df in batch_download(syms[i:i+50],period="6mo").items():
            try: closes[sy]=df["Close"].squeeze().astype(float).dropna()
            except Exception: pass
    nifty=get_yf().Ticker("^NSEI").history(period="6mo",interval="1d",auto_adjust=True)["Close"].astype(float)
    def fwd(ser,date,h):
        idx=ser.index.tz_localize(None) if getattr(ser.index,"tz",None) is not None else ser.index
        pos=idx.searchsorted(pd.Timestamp(date),side="right")-1
        return None if (pos<0 or pos+h>=len(ser)) else (float(ser.iloc[pos+h])/float(ser.iloc[pos])-1)*100
    rows=[]
    for e in log:
        for p in e["picks"]:
            sr=closes.get(p["s"])
            ...
            for h in (1,3,5,10):
                a=fwd(sr,e["date"],h); b=fwd(nifty,e["date"],h)
                if a is not None and b is not None: rows.append((p["tags"],h,a,a-b))
    def agg(sel):
        out={}
        for h in (1,3,5,10):
            x=[(a,b) for t,hh,a,b in rows if hh==h and sel(t)]
            if x: out[str(h)]={"n":len(x),"avg":round(sum(a for a,_ in x)/len(x),2),"excess":round(sum(b for _,b in x)/len(x),2),
                               "win":round(100*sum(1 for a,_ in x if a>0)/len(x)),"beat":round(100*sum(1 for _,b in x if b>0)/len(x))}
        return out
    tags=["trend_template","vcp","rs_top20pct","breakout","delivery_accum","supertrend","macd"]
    return {"status":"success","days":len(log),"first":log[0]["date"],"overall":agg(lambda t:True),
            "by_signal":{k:agg(lambda t,k=k:k in t) for k in tags}}

@app.route("/scorecard")
@require_auth
def scorecard():
    try:
        if _score_cache["data"] is None or time.time()-_score_cache["t"]>1800:
            _score_cache["data"]=sanitise(compute_scorecard()); _score_cache["t"]=time.time()
...
def _do_scan():
    ...
    try:
        ...
        for batch_start in range(0, len(WATCHLIST), 50):
            ...
            batch_data = batch_download(batch, period="1y", interval="1d")
        ...
        _finalize_scan_context(results, regime)
        ...
        set_full_scan_snapshot({r["symbol"]: {"score":r.get("score"), "ret_20d":r.get("ret_20d"), "rs_raw":r.get("rs_raw"), "rs_rank":r.get("rs_rank"),
...
def _last_close_ts():
    """Epoch of the most recent 15:30 IST weekday close (holidays ignored)."""
    n=datetime.now(IST); d=n.replace(hour=15,minute=30,second=0,microsecond=0)
    if n<d: d-=timedelta(days=1)
    while d.weekday()>=5: d-=timedelta(days=1)
    return d.timestamp()

def _hot_stale(res):
    """Hot Movers must not serve yesterday's (or a pre-close) snapshot as if it were live."""
    gen=(res or {}).get("generated_ts",0)
...
# ── FUNDAMENTALS ──────────────────────────────────────────────────────────
# ── Fundamentals cache: quarterly data barely changes, and Yahoo's quoteSummary (.info) endpoint is the
# one it throttles hardest from cloud IPs (HTTP 401 "Invalid Crumb"). Cache successes for 24h so every
# run only needs to fetch what's missing, and fall back to entries up to 7 days old when Yahoo refuses.
_FUND_PATH=os.environ.get("FUND_CACHE_PATH","/tmp/mm_fund_cache.json"); _fund_lock=threading.Lock()
try:
    with open(_FUND_PATH) as _f: _fund_cache=json.load(_f)
except Exception: _fund_cache={}
def _fund_cache_get(sym, max_age):
    e=_fund_cache.get(sym)
    return dict(e["r"]) if e and time.time()-e["t"]<=max_age else None
def _fund_cache_save():
    try:
        with _fund_lock:
            with open(_FUND_PATH,"w") as f: json.dump(_fund_cache,f)
    except Exception as e: print(f"[FUND CACHE ERR] {e}")
def _warm_yahoo_session():
    """Fetch Yahoo's cookie/crumb once, serially, before the threaded scan (avoids a crumb stampede -> HTTP 429)."""
    global _info_blocked_until
    for i in range(2):
        try:
            info=get_yf().Ticker("RELIANCE.NS").info
            if info and len(info)>=10: print("[LT] Yahoo session warm"); return True
        except Exception as e: print(f"[LT] warm-up {i+1}/2 failed: {str(e)[:90]}")
        time.sleep(3)
    _info_blocked_until=time.time()+1800
    print("[LT] Yahoo .info unavailable — going straight to financial statements")
    return False
def _ensure_target(r):
    """Yahoo's analyst targets are gone with the quote API. Estimate a 12-month fair value instead (clearly flagged
    target_source="model"): growth-adjusted P/E on forward EPS, or P/B scaled by ROE for banks/NBFCs. Bounded to 0.75x-1.4x of price."""
    try:
        if not r or r.get("analyst_target") or not r.get("cmp"): return r
        px=r["cmp"]; eps=r.get("eps_trailing"); roe=r.get("roe"); pb=r.get("pb")
        g=r.get("earnings_growth"); g=g if g is not None else r.get("revenue_growth")
        sec=(r.get("sector") or "").lower(); fin=any(k in sec for k in ("financ","bank","nbfc","insur"))
        val=None
        if fin and pb and pb>0 and roe and roe>0:
            val=px/pb*max(0.8,min(4.0,roe/12.0))                          # fair P/B ~ ROE / 12% cost of equity
        elif eps and eps>0:
            gg=max(0.0,min(25.0,g if g is not None else 8.0))
            val=eps*(1+gg/100)*max(10.0,min(25.0,8.5+1.5*gg))             # forward EPS x growth-adjusted fair P/E
        if val:
            t=round(max(0.75*px,min(1.4*px,val)),2)
            r["analyst_target"]=t; r["target_source"]="model"; r["analyst_upside"]=round((t-px)/px*100,1)
    except Exception: pass
    return r

def fetch_fundamentals(sym, daily_pre=None):
    c=_fund_cache_get(sym,24*3600)
    if c is not None and c.get("cmp"): return _ensure_target(c)
    r=_fetch_fundamentals_live(sym, daily_pre)
    if r:
        _ensure_target(r)
        _fund_cache[sym]={"t":time.time(),"r":r}
        return dict(r)
    st=_fund_cache_get(sym,7*86400)
    if st is not None and st.get("cmp"): st["stale"]=True; return _ensure_target(st)
    return None

# ── Fallback fundamentals from the financial STATEMENTS (Yahoo "fundamentals-timeseries" endpoint) ─────────
# Yahoo has disabled API-level access to its quote data (the crumb-protected .info / quoteSummary endpoint:
# HTTP 401 "API-level access to Yahoo Finance quotes data has been disabled"). Statements + price history use
# different endpoints, so we rebuild the metrics we score on (P/E, ROE, D/E, growth, FCF, margins, P/B) from them.
# Not available this way: analyst targets/recommendations, forward P/E, insider %, beta (those checks just skip).
_info_fail_streak=0; _info_blocked_until=0
_SECTOR_OF={}
for _sn,_ss in SECTOR_STOCKS.items():
    for _x in _ss: _SECTOR_OF.setdefault(_x,_sn)

def _stmt_df(df):
    try:
        if df is None or df.empty: return None
        return df.sort_index(axis=1,ascending=False)   # latest period first
    except Exception: return None
def _stmt_val(df, names, col=0):
    try:
        if df is None or df.shape[1]<=col: return None
        for n in names:
            if n in df.index:
                v=df.loc[n].iloc[col]
    ...
    return None
def _stmt_sum(df, names, n=4):
    try:
        if df is None or df.shape[1]<n: return None
        for nm in names:
            if nm in df.index:
                vals=df.loc[nm].iloc[:n]
                return None if vals.isna().any() else float(vals.sum())
    except Exception: pass
    return None
def _yoy(q, a, names):
    """Growth: latest quarter vs same quarter last year if 5 quarters exist, else latest annual vs previous."""
    try:
        if q is not None and q.shape[1]>=5:
            a0=_stmt_val(q,names,0); a4=_stmt_val(q,names,4)
            if a0 is not None and a4 not in (None,0): return (a0-a4)/abs(a4)
        if a is not None and a.shape[1]>=2:
            a0=_stmt_val(a,names,0); a1=_stmt_val(a,names,1)
            if a0 is not None and a1 not in (None,0): return (a0-a1)/abs(a1)
    except Exception: pass
    return None

def _info_from_statements(sym, tk, h=None):
    if h is None or len(h)<20: h=tk.history(period="1y",interval="1d",auto_adjust=True)
    if h is None or len(h)<20: return None
    _c=h["Close"].dropna()
    if len(_c)<20: return None
    px=float(_c.iloc[-1]); hi=float(h["High"].dropna().max()); lo=float(h["Low"].dropna().min())
    def _safe(fn):
        try: return _stmt_df(fn())
        except Exception: return None
    _ex=ThreadPoolExecutor(max_workers=3)      # 3 requests at once instead of one after another
    _fq=_ex.submit(_safe, lambda: tk.quarterly_income_stmt)
    _fb=_ex.submit(_safe, lambda: tk.balance_sheet)
    _fc=_ex.submit(_safe, lambda: tk.cashflow)
    def _r(fu):
        try: return fu.result(timeout=15)          # give up reading a stuck statement after 15s
        except Exception: return None
    qi,ab,ac=_r(_fq),_r(_fb),_r(_fc)
    ai=None
    if qi is None or qi.shape[1]<5:                 # annual only as a fallback — same 15s bound, not a bare synchronous call
        ai=_r(_ex.submit(_safe, lambda: tk.income_stmt))
    _ex.shutdown(wait=False)                        # don't block here if a request is genuinely hung — move on
    NI=["Net Income","Net Income Common Stockholders"]; REV=["Total Revenue","Operating Revenue"]
    EPS=["Diluted EPS","Basic EPS"]; GP=["Gross Profit"]; OP=["Operating Income","EBIT"]
    def ttm(names): 
        v=_stmt_sum(qi,names)
        return v if v is not None else _stmt_val(ai,names)
    ni,rev,eps,gp,opi=ttm(NI),ttm(REV),ttm(EPS),ttm(GP),ttm(OP)
    equity=_stmt_val(ab,["Stockholders Equity","Common Stock Equity","Total Equity Gross Minority Interest"])
    debt=_stmt_val(ab,["Total Debt"]); assets=_stmt_val(ab,["Total Assets"])
    ca=_stmt_val(ab,["Current Assets"]); cl=_stmt_val(ab,["Current Liabilities"])
    fcf=_stmt_val(ac,["Free Cash Flow"])
    if fcf is None:
        ocf=_stmt_val(ac,["Operating Cash Flow","Cash Flow From Continuing Operating Activities"]); cap=_stmt_val(ac,["Capital Expenditure"])
        if ocf is not None and cap is not None: fcf=ocf+cap   # capex is reported negative
    shares=(ni/eps) if (ni and eps and eps>0 and ni>0) else None
    d=lambda a,b: (a/b) if (a is not None and b not in (None,0)) else None
    info={"currentPrice":px,"longName":sym,"sector":_SECTOR_OF.get(sym,"—"),
          "marketCap":px*shares if shares else None,
          "trailingPE":(px/eps) if (eps and eps>0) else None,
          "priceToBook":(px/(equity/shares)) if (equity and equity>0 and shares) else None,
          "returnOnEquity":d(ni,equity) if (equity and equity>0) else None,
          "returnOnAssets":d(ni,assets),
          "debtToEquity":(debt/equity*100) if (debt is not None and equity and equity>0) else None,
          "currentRatio":d(ca,cl),"grossMargins":d(gp,rev),"operatingMargins":d(opi,rev),"profitMargins":d(ni,rev),
          "earningsGrowth":_yoy(qi,ai,NI),"revenueGrowth":_yoy(qi,ai,REV),
          "trailingEps":eps,"freeCashflow":fcf,"fiftyTwoWeekHigh":hi,"fiftyTwoWeekLow":lo}
    if info["trailingPE"] is None and info["returnOnEquity"] is None: return None   # nothing usable
    return info

def _fetch_fundamentals_live(sym, daily_pre=None):
    global _info_fail_streak, _info_blocked_until
    try:
        yf=get_yf(); tk=yf.Ticker(get_ns(sym))
        info=None
        if time.time()>_info_blocked_until:
            try: info=yf_retry(lambda: tk.info)
            except Exception: info=None
            if info and len(info)>=10: _info_fail_streak=0
            else:
                info=None; _info_fail_streak+=1
                if _info_fail_streak>=3:
                    _info_blocked_until=time.time()+1800
                    print("[FUND] Yahoo .info is blocked — using financial statements for the next 30 min")
        if not info:
            info=_info_from_statements(sym, tk, daily_pre)
        if not info or len(info)<10: return None
        cmp=info.get("currentPrice") or info.get("regularMarketPrice")
        if not cmp or cmp!=cmp: return None
        ...
        if cached and cached.get("rsi") is not None:
            ...
            r["trend_template"]=bool(cached.get("trend_template")); r["stage4"]=bool(cached.get("stage4"))
        else:
            try:
                daily=daily_pre if (daily_pre is not None and len(daily_pre)>=60) else tk.history(period="1y",interval="1d",auto_adjust=True)
                ...
                r["trend_template"]=bool(tech.get("trend_template")) if tech else False; r["stage4"]=bool(tech.get("stage4")) if tech else False
            except Exception:
...
def score_fundamentals(f):
    if not f: return 0,[]
    score=0; signals=[]
    _sec=(f.get("sector") or "").lower(); fin=any(k in _sec for k in ("financ","bank","nbfc","insur"))   # banks/NBFCs: leverage is the business, judge on P/B + ROE
    pe=f.get("pe")
    if pe:
    ...
    if fin and f.get("pb"):
        pbv=f["pb"]
        if pbv<=1.5: score+=1.0; signals.append(f"P/B {pbv} — reasonable for a financial ✓")
        elif pbv>4:  score-=0.5; signals.append(f"P/B {pbv} — rich for a financial ✗")
    ...
    if upside and na>=3:
        if upside>=30:   score+=1.0; signals.append(f"Analyst target +{upside}% ({na} analysts) ✓")
        elif upside>=15: score+=0.75; signals.append(f"Analyst target +{upside}% ({na} analysts) ✓")
    ...
    if f.get("trend_template"): score+=1.5; signals.append("Stage 2 uptrend — value + momentum aligned ✓")
    elif f.get("stage4"):      score-=2.0; signals.append("Below a falling 200-day average — value-trap risk ✗")
...
# ── LT SCAN ───────────────────────────────────────────────────────────────
def _apply_sector_context(res):
    """Judge P/E against the stock's own sector median (>=5 peers) instead of one absolute scale."""
    try:
        by={}
        for r in res:
            if r.get("pe") and r["pe"]>0 and r.get("sector"): by.setdefault(r["sector"],[]).append(r["pe"])
        med={k:sorted(v)[len(v)//2] for k,v in by.items() if len(v)>=5}
        for r in res:
            m=med.get(r.get("sector")); pe=r.get("pe")
            if not m or not pe or pe<=0: continue
            if pe<=m*0.75:  r["score"]=min(23,round(r["score"]+1.0,1)); r["signals"].append(f"P/E {pe} vs sector median {round(m,1)} — cheap vs peers ✓")
            elif pe>=m*1.6: r["score"]=max(0,round(r["score"]-1.0,1)); r["signals"].append(f"P/E {pe} vs sector median {round(m,1)} — pricey vs peers ✗")
            sc=r["score"]; r["grade"]="A+" if sc>=11 else "A" if sc>=8.5 else "B" if sc>=6 else "C" if sc>=3.5 else "D"
        res.sort(key=lambda x:x.get("score",0),reverse=True)
    except Exception as e: print(f"[SECTOR CTX ERR] {e}")
...
def _do_lt_scan():
    ...
    try:
        ...
        all_results = []
        batch_size  = 40
        failed = []   # symbols that returned no usable fundamentals (Yahoo rate-limit / bad ticker)
        blocked = 0; blocked_abort = False
        # Hard time budget for the WHOLE scan (batches + retry pass). Whatever the cause of slowness —
        # Yahoo throttling harder, a bad batch, a slow retry pass — this guarantees the user gets an
        # answer instead of watching "scanning..." for 30 minutes. Override with env LT_SCAN_DEADLINE_SEC.
        scan_start = time.time()
        SCAN_DEADLINE = float(os.environ.get("LT_SCAN_DEADLINE_SEC", 360))   # 6 min
        timed_out = False
        if any(_fund_cache_get(x, 24*3600) is None for x in lt_list): _warm_yahoo_session()

        daily_map = {}          # one bulk price download (8 requests) instead of 360 single ones
        try:
            for _i in range(0, len(lt_list), 50): daily_map.update(batch_download(lt_list[_i:_i+50], period="1y", interval="1d"))
            print(f"[LT] Prices prefetched: {len(daily_map)}/{len(lt_list)}")
        except Exception as _e: print(f"[LT] price prefetch failed: {_e}")

        for batch_num, batch_start in enumerate(range(0, len(lt_list), batch_size)):
            if time.time() - scan_start > SCAN_DEADLINE:
                timed_out = True
                print(f"[LT] Time budget ({int(SCAN_DEADLINE)}s) reached — stopping before batch {batch_num+1}, returning {len(all_results)} so far")
                break
            ...
            ex = ThreadPoolExecutor(max_workers=(6 if time.time() < _info_blocked_until else 3))
            ...
            try:
                for f in as_completed(futures, timeout=90):
                    ...
                    except Exception as _ex: failed.append(futures[f]); print(f'[WARN] {_ex}')
            except Exception:
                pass  # as_completed's own 90s ceiling hit — whatever didn't finish is abandoned below
            for f in futures:
                if f not in got: failed.append(futures[f])
            ex.shutdown(wait=False)   # do NOT wait for any still-stuck symbols — that's the 30-min stall this replaces
            gc.collect()
            time.sleep(1)  # spread requests over time — gentler on Yahoo's rate limiter
            _fund_cache_save()
            if len(all_results) - valid_before == 0 and len(batch) >= 10:   # whole batch rejected -> Yahoo is blocking us
                blocked += 1
                print(f"[LT] Batch fully rejected by Yahoo ({blocked}/3)")
                if blocked >= 3: blocked_abort = True
                else: time.sleep(45)
            else: blocked = 0
            ...
            with _jobs_lock:
                ...
                _jobs["lt_scan"]["running"] = more and time.time()-scan_start<=SCAN_DEADLINE
        ...
        if failed and len(failed) <= len(lt_list)*0.6 and not timed_out and time.time()-scan_start<SCAN_DEADLINE:
            print(f"[LT] Retrying {len(failed)} failed symbols")
            time.sleep(3)
            retry_failed = list(failed); failed = []
            for i in range(0, len(retry_failed), 20):
                if time.time() - scan_start > SCAN_DEADLINE:
                ...
                try:
                    for f in as_completed(futs, timeout=30):
                        got.add(f); sym_=futs[f]
                        try:
                            r=f.result(timeout=10)
                            if r and (r.get("pe") or r.get("roe")):
                                sc_, sg_ = score_fundamentals(r); r["score"]=sc_; r["signals"]=sg_
                                r["grade"]="A+" if sc_>=11 else "A" if sc_>=8.5 else "B" if sc_>=6 else "C" if sc_>=3.5 else "D"
                                all_results.append(sanitise(r))
                            else: failed.append(sym_)
                        except Exception: failed.append(sym_)
                except Exception: pass   # 30s group ceiling hit — whatever didn't finish is abandoned, not waited on
                for f in futs:
                    if f not in got: failed.append(futs[f])
                ex.shutdown(wait=False)
                time.sleep(1)
            all_results.sort(key=lambda x: x.get("score", 0), reverse=True)
        print(f"[LT] Complete — {len(all_results)} valid, {len(failed)} failed of {len(lt_list)}{' (time budget reached)' if timed_out else ''}")
        _apply_sector_context(all_results)
        with _jobs_lock:
            _jobs["lt_scan"]["result"]["top15"] = all_results[:20]
            _jobs["lt_scan"]["result"]["failed"] = len(failed)
            if not all_results:
                _jobs["lt_scan"]["result"]["status"] = "empty"
                _jobs["lt_scan"]["result"]["message"] = "Yahoo Finance is rejecting fundamentals requests from the server right now (HTTP 401 crumb / rate limit) and nothing is cached yet. Wait 15-30 minutes and press Refresh."
            elif blocked_abort:
                _jobs["lt_scan"]["result"]["message"] = f"Yahoo is rejecting fundamentals requests right now — showing {len(all_results)} stocks (cached data). Press Refresh in 15-30 min to fill in the rest."
            elif timed_out:
                _jobs["lt_scan"]["result"]["message"] = f"Hit the {int(SCAN_DEADLINE//60)}-min time budget — showing {len(all_results)}/{len(lt_list)} stocks. Press Refresh to pick up where this left off (already-scanned stocks are cached for 24h, so a repeat run should be faster)."
            _jobs["lt_scan"]["result"]["blocked"] = blocked_abort
            _jobs["lt_scan"]["result"]["timed_out"] = timed_out
            _jobs["lt_scan"]["result"]["generated_ts"] = time.time()
            _jobs["lt_scan"]["result"]["partial"] = False
            _jobs["lt_scan"]["running"] = False
        ...
        for url in urls:
            try:
                ...
                for item in items[:100]:
                    ...
                    purpose= item.get("purpose") or item.get("bm_purpose") or item.get("bm_desc") or item.get("description","")
...
def _nse_rows(payload):
    """NSE returns either {"data":[...]}, a bare list, or grouped {"allSec":{"data":[...]},"NIFTY":{"data":[...]},...}.
    The old code only understood the first form, so Top Gainers/Losers (grouped) always came back empty."""
    if isinstance(payload,list): return [x for x in payload if isinstance(x,dict)]
    if not isinstance(payload,dict): return []
    if isinstance(payload.get("data"),list): return [x for x in payload["data"] if isinstance(x,dict)]
    rows=[]; seen=set()
    order=[k for k in ("allSec","NIFTY","FOSec","SecGtr20","SecLwr20","BANKNIFTY","NIFTYNEXT50") if k in payload]+[k for k in payload if k not in ("allSec","NIFTY","FOSec","SecGtr20","SecLwr20","BANKNIFTY","NIFTYNEXT50")]
    for k in order:
        g=payload.get(k); d=g.get("data") if isinstance(g,dict) else (g if isinstance(g,list) else None)
        for i in (d or []):
            if isinstance(i,dict) and i.get("symbol") and i["symbol"] not in seen: seen.add(i["symbol"]); rows.append(i)
    return rows
def _nf(v):
    try: return float(str(v).replace(",","").strip())
    except Exception: return 0.0
def _deal_row(i):
    return {"symbol":str(i.get("symbol","")).strip().upper(),"client":i.get("clientName") or i.get("client") or "—",
            "buy_sell":i.get("buySell") or i.get("buy_sell") or "—",
            "quantity":_nf(i.get("qty") or i.get("quantityTraded") or i.get("tradedQty") or i.get("totalTradedVolume")),
            "price":_nf(i.get("watp") or i.get("tradePrice") or i.get("avgPrice") or i.get("close"))}

_last_fii_dii=None
def parse_fii_dii(fd, now=None):
    """NSE's fiidiiTradeReact returns TWO rows (category 'DII' and 'FII/FPI'), values in Rs crore as strings.
    The old code read only row 0 for both, so FII/DII came back empty. Keeps the last good reading as a fallback."""
    global _last_fii_dii
    def num(v):
        try: return float(str(v).replace(",","").strip())
        except Exception: return None
    empty={"fii_net":None,"dii_net":None,"fii_buy":None,"fii_sell":None,"dii_buy":None,"dii_sell":None}
    if isinstance(fd,list) and fd:
        rows=[r for r in fd if isinstance(r,dict)]
        fr_=next((r for r in rows if any(k in str(r.get("category","")).upper() for k in ("FII","FPI"))),None)
        dr_=next((r for r in rows if "DII" in str(r.get("category","")).upper() and r is not fr_),None)
        if fr_ or dr_:
            g=lambda r,*ks: next((num(r.get(k)) for k in ks if r and num(r.get(k)) is not None),None)
            out={"fii_buy":g(fr_,"buyValue","fiiBuyValue"),"fii_sell":g(fr_,"sellValue","fiiSellValue"),"fii_net":g(fr_,"netValue","fiiNetValue"),
                 "dii_buy":g(dr_,"buyValue","diiBuyValue"),"dii_sell":g(dr_,"sellValue","diiSellValue"),"dii_net":g(dr_,"netValue","diiNetValue"),
                 "date":(fr_ or dr_).get("date") or (now.strftime("%d-%b-%Y") if now else "")}
            if out["fii_net"] is not None or out["dii_net"] is not None:
                _last_fii_dii=out; return out
    if _last_fii_dii:                      # NSE didn't answer this time -> show the last known reading (with its own date)
        return dict(_last_fii_dii)
    return empty
...
def _do_hot_movers():
    try:
        ...
        nse_endpoints = {
            ...
            "circuits":"https://www.nseindia.com/api/live-analysis-price-band-hitter",
        }
        def fetch_nse_endpoint(key_url):
            ...
            try:
                ...
                if r.ok:
                    j = r.json()
                    items = _nse_rows(j)
                    if key=="circuits" and not items and isinstance(j,dict):   # grouped as upper -> AllSec -> data (or similar)
                        for _grp in ("upper","Upper","upperCircuit"):
                            _g=j.get(_grp)
                            if isinstance(_g,dict):
                                for _sub in ("AllSec","allSec","data"):
                                    _d=_g.get(_sub)
                                    if isinstance(_d,dict): _d=_d.get("data")
                                    if isinstance(_d,list) and _d: items=[x for x in _d if isinstance(x,dict)]; break
                            if items: break
                    out=[]
                    for i in items:
                        sym=str(i.get("symbol","")).strip().upper()
                        if not sym: continue
                        ltp=_nf(i.get("lastPrice") or i.get("ltp")); prev=_nf(i.get("previousPrice") or i.get("previousClose") or i.get("prev_price"))
                        pc=i.get("pChange") if i.get("pChange") is not None else (i.get("percentChange") if i.get("percentChange") is not None else i.get("perChange"))
                        pct=_nf(pc) if pc not in (None,"") else (round((ltp-prev)/prev*100,2) if prev else 0.0)
                        out.append({"symbol":sym,"ltp":ltp,"change_pct":pct,
                            "volume":_nf(i.get("totalTradedVolume") or i.get("quantityTraded") or i.get("trade_quantity") or i.get("tradedQuantity") or i.get("volume")),
                            "prev_close":prev})
                    if key=="gainers": out.sort(key=lambda x:x["change_pct"],reverse=True)
                    elif key=="losers": out.sort(key=lambda x:x["change_pct"])
                    elif key=="active": out.sort(key=lambda x:x["volume"],reverse=True)
                    return key, out[:20]
                else: print(f"[HOT] {key} HTTP {r.status_code}")
            except Exception as _ex: print(f'[WARN] {key}: {_ex}')
            if key=="losers":                      # NSE has used both "losers" and "loosers" across deployments — try the other spelling
                try:
                    alt="https://www.nseindia.com/api/live-analysis-variations?index=losers"
                    r=session.get(alt,headers=NSE_HEADERS,timeout=10)
                    if r.ok:
                        items=_nse_rows(r.json())
                        out=[{"symbol":str(i.get("symbol","")).strip().upper(),"ltp":_nf(i.get("lastPrice") or i.get("ltp")),
                              "change_pct":_nf(i.get("pChange") if i.get("pChange") is not None else i.get("percentChange")),
                              "volume":_nf(i.get("totalTradedVolume") or i.get("quantityTraded")),
                              "prev_close":_nf(i.get("previousPrice") or i.get("previousClose"))} for i in items if i.get("symbol")]
                        out.sort(key=lambda x:x["change_pct"])
                        if out: return key, out[:20]
                except Exception as _ex2: print(f'[WARN] losers alt spelling: {_ex2}')
            return key, []
        ...
        # Bulk / block deals — today's snapshot first, legacy URLs as fallback
        bulk=[]; block=[]
        try:
            r=session.get("https://www.nseindia.com/api/snapshot-capital-market-largedeal",headers=NSE_HEADERS,timeout=10)
            if r.ok:
                j=r.json()
                bulk=[_deal_row(i) for i in (j.get("BULK_DEALS_DATA") or []) if isinstance(i,dict) and i.get("symbol")][:20]
                block=[_deal_row(i) for i in (j.get("BLOCK_DEALS_DATA") or []) if isinstance(i,dict) and i.get("symbol")][:20]
            else: print(f"[HOT] deals HTTP {r.status_code}")
        except Exception as _ex: print(f'[WARN] deals snapshot: {_ex}')
        if not bulk and not block:
            for key,url in [("bulk","https://www.nseindia.com/api/bulk-deals"),("block","https://www.nseindia.com/api/block-deals")]:
                try:
                    r=session.get(url,headers=NSE_HEADERS,timeout=8)
                    if r.ok:
                        cleaned=[_deal_row(i) for i in _nse_rows(r.json())[:20] if i.get("symbol")]
                        if key=="bulk": bulk=cleaned
                        else: block=cleaned
                except Exception as _ex: print(f'[WARN] {key} deals: {_ex}')
        ...
        def check_stock(sym):
            try:
                daily=yf.Ticker(get_ns(sym)).history(period="1y",interval="1d",auto_adjust=True)
                ...
                base=volume.iloc[-51:-1] if len(volume)>=52 else volume.iloc[:-1]
                avg_vol=float(base.mean())
                today_vol=float(volume.iloc[-1])
                frac=1.0
                if _is_market_open():
                    _n=datetime.now(IST); frac=max(0.15,min(1.0,((_n.hour*60+_n.minute)-(9*60+15))/375.0))
                rel=round((today_vol/frac)/avg_vol,1) if avg_vol>0 else 1.0   # projected to a full session
                if rel<1.8: return None
                rsi_val=None
                try: rsi_val=round(float(ta.momentum.RSIIndicator(close=close,window=14).rsi().iloc[-1]),1)
                except Exception as _ex: print(f'[WARN] {_ex}')
                w52h=float(close.iloc[-252:].max()); w52l=float(close.iloc[-252:].min())
        ...
        try:
            ...
            for _try in range(2):
                ...
                if isinstance(fd,list) and fd: break
                time.sleep(2)
            fii_dii=parse_fii_dii(fd, now)
            print(f"[FII] net={fii_dii['fii_net']} DII={fii_dii['dii_net']} date={fii_dii.get('date')}")
        except Exception as e:
            print(f"[FII ERR] {e}")
            fii_dii=parse_fii_dii(None)

        print(f"[HOT] gainers={len(gainers)} losers={len(losers)} active={len(active)} circuits={len(circuits)} bulk={len(bulk)} block={len(block)} shockers={len(shockers)} results={len(upcoming_results)} fii_net={fii_dii.get('fii_net')}")
        set_job_done("hot_movers",sanitise({
            "status":"success","generated_ts":time.time(),"fii_dii":fii_dii,"scan_time":now.strftime("%I:%M %p IST"),"date":now.strftime("%d %b %Y, %A"),
...
def analyse(symbol):
    try:
        ...
        daily=_ticker.history(period="1y",interval="1d",auto_adjust=True)
        df=daily if (daily is not None and len(daily)>=26) else intra
        ind=compute_indicators(df,intra)
        ...
        regime=get_job("scan").get("result",{}).get("market_regime") or fetch_nifty_regime()
        ind.update(compute_advanced_signals(daily, regime))
        ind["rs_rank"] = rs_percentile(ind.get("rs_raw"))
        ind["delivery_pct"] = get_delivery_pct(sym)
        ind["fno_buildup"] = get_fno_buildup(sym)
        ...
        return jsonify(sanitise({
            ...
            "rs_score":ind.get("rs_score"),"ret_20d":ind.get("ret_20d"),"rs_rank":ind.get("rs_rank"),"trend_template":ind.get("trend_template"),"tt_score":ind.get("tt_score"),"stage4":ind.get("stage4"),"mom_12_1":ind.get("mom_12_1"),
...
def lt_scan():
    ...
    job = get_job("lt_scan")
    result = job.get("result")
    if result and result.get("status")=="empty" and not job.get("running", False) and time.time()-result.get("generated_ts",0)>120:
        # Last scan came back empty (Yahoo throttling) — don't serve it forever; start a fresh one.
        with _jobs_lock: _jobs["lt_scan"]["result"]=None
        result=None
    if result:
...
@require_auth
def hot_diag():
    """Open in the browser: shows, per NSE endpoint, the HTTP status, JSON shape and how many rows we can parse —
    tells a genuinely empty market apart from a blocked / changed endpoint."""
    import requests as rq
    out={}; sess=rq.Session()
    try: out["session_prime"]=sess.get("https://www.nseindia.com",headers=NSE_HEADERS,timeout=8).status_code
    except Exception as e: out["session_prime"]="FAIL "+str(e)[:80]
    urls={"gainers":"https://www.nseindia.com/api/live-analysis-variations?index=gainers",
          ...
          "large_deals":"https://www.nseindia.com/api/snapshot-capital-market-largedeal",
          "bulk_legacy":"https://www.nseindia.com/api/bulk-deals","block_legacy":"https://www.nseindia.com/api/block-deals",
    ...
    for k,u in urls.items():
        try:
            r=sess.get(u,headers=NSE_HEADERS,timeout=10); info={"http":r.status_code}
            try:
                j=r.json(); info["shape"]=f"list[{len(j)}]" if isinstance(j,list) else "dict keys: "+", ".join(list(j.keys())[:8])
                info["parsed_rows"]=len(_nse_rows(j))
            except Exception: info["body_starts"]=r.text[:80]
            out[k]=info
        except Exception as e: out[k]="FAIL "+str(e)[:100]
    return jsonify(out)

@app.route("/yf-diag")
@require_auth
def yf_diag():
    """Open in the browser to see exactly what Yahoo allows from this server right now."""
    out={}
    try:
        yf=get_yf(); out["yfinance_version"]=getattr(yf,"__version__","?"); t=yf.Ticker("RELIANCE.NS")
        try: h=t.history(period="5d"); out["price_history"]="OK" if h is not None and len(h)>0 else "EMPTY"
        except Exception as e: out["price_history"]="FAIL: "+str(e)[:150]
        try:
            i=t.info; out["fundamentals_info"]=f"OK ({len(i)} fields)" if i and len(i)>10 else "EMPTY / blocked"
        except Exception as e: out["fundamentals_info"]="FAIL: "+str(e)[:200]
        try:
            q=t.quarterly_income_stmt; b=t.balance_sheet
            out["statements_fallback"]=f"OK (income {q.shape[1]} quarters, balance {b.shape[1]} years)" if q is not None and not q.empty and b is not None and not b.empty else "EMPTY / blocked"
            inf=_info_from_statements("RELIANCE",t); out["sample_rebuilt_metrics"]={k:(round(v,3) if isinstance(v,float) else v) for k,v in (inf or {}).items() if k in ("trailingPE","priceToBook","returnOnEquity","debtToEquity","earningsGrowth","revenueGrowth","freeCashflow")}
        except Exception as e: out["statements_fallback"]="FAIL: "+str(e)[:200]
...
def hot_movers():
    ...
    result = job.get("result")
    if result and result.get("status") == "success":
        stale = _hot_stale(result)
        if stale and not job.get("running", False):      # old snapshot -> refresh in background, flag it to the UI
            if set_job_running("hot_movers"):
                threading.Thread(target=_do_hot_movers, daemon=True).start()
        out = dict(result); out["stale"] = stale; out["market_open"] = _is_market_open()
        return jsonify(out)
    if not job.get("running", False):
        if set_job_running("hot_movers"):
            threading.Thread(target=_do_hot_movers, daemon=True).start()
