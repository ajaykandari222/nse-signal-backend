...
def _info_from_statements(sym, tk, h=None):
    ...
    with ThreadPoolExecutor(max_workers=3) as _ex:      # 3 requests at once instead of one after another
        ...
        qi,ab,ac=_fq.result(),_fb.result(),_fc.result()
    ai=_safe(lambda: tk.income_stmt) if (qi is None or qi.shape[1]<5) else None   # annual only as a fallback
...
def _do_lt_scan():
    ...
    try:
        ...
        for batch_num, batch_start in enumerate(range(0, len(lt_list), batch_size)):
            ...
            with ThreadPoolExecutor(max_workers=(6 if time.time() < _info_blocked_until else 3)) as ex:
        ...
        if failed and len(failed) <= len(lt_list)*0.6:
            print(f"[LT] Retrying {len(failed)} failed symbols")
            time.sleep(5)
            retry_failed = list(failed); failed = []
            for i in range(0, len(retry_failed), 20):
                with ThreadPoolExecutor(max_workers=4) as ex:
                    for sym_, r in zip(retry_failed[i:i+20], ex.map(fetch_fundamentals, retry_failed[i:i+20])):
                        if r and (r.get("pe") or r.get("roe")):
                ...
                time.sleep(2)
            all_results.sort(key=lambda x: x.get("score", 0), reverse=True)
        print(f"[LT] Complete — {len(all_results)} valid, {len(failed)} failed of {len(lt_list)}")
