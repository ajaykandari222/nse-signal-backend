...
def _warm_yahoo_session():
    ...
    for i in range(3):
        ...
        except Exception as e: print(f"[LT] warm-up {i+1}/3 failed: {str(e)[:90]}")
        time.sleep(6+i*4)
    return False
def fetch_fundamentals(sym):
    c=_fund_cache_get(sym,24*3600)
    if c is not None: return c
    r=_fetch_fundamentals_live(sym)
    if r:
        _fund_cache[sym]={"t":time.time(),"r":r}
        return dict(r)
    st=_fund_cache_get(sym,7*86400)
    if st is not None: st["stale"]=True; return st
    return None
...
def _info_from_statements(sym, tk):
    h=tk.history(period="1y",interval="1d",auto_adjust=True)
    if h is None or len(h)<20: return None
    px=float(h["Close"].iloc[-1]); hi=float(h["High"].max()); lo=float(h["Low"].min())
    qi=_stmt_df(tk.quarterly_income_stmt); ai=_stmt_df(tk.income_stmt)
    ab=_stmt_df(tk.balance_sheet); ac=_stmt_df(tk.cashflow)
...
def _fetch_fundamentals_live(sym):
    ...
    try:
        ...
        if not info:
            info=_info_from_statements(sym, tk)
